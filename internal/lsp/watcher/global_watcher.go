...
// to use .sgptignore and .gitignore files for filtering file events
func setupIgnoreSystem(root string) error {
	...
	// Load .sgptignore file if it exists
	sgptignorePath := filepath.Join(root, ".sgptignore")
	if _, err := os.Stat(sgptignorePath); err == nil {
		if err := im.LoadIgnoreFile(sgptignorePath); err != nil {
			slog.Warn("lsp watcher: Failed to load .sgptignore file", "error", err)
		}
	}
