...
import (
	...
	"github.com/skorpland/sgptcoder/internal/env"
...
func PushPopSgptEnv() func() {
	found := []string{}
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "SGPT_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "SGPT_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("SGPT_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("SGPT_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
...
func globalConfig() string {
	...
	// for windows, it should be in `%LOCALAPPDATA%/sgpt/`
	// for linux and macOS, it should be in `$HOME/.config/sgpt/`
...
func GlobalConfigData() string {
	...
	// for windows, it should be in `%LOCALAPPDATA%/sgpt/`
	// for linux and macOS, it should be in `$HOME/.local/share/sgpt/`
	if runtime.GOOS == "windows" {
		localAppData := os.Getenv("LOCALAPPDATA")
		if localAppData == "" {
			localAppData = filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local")
