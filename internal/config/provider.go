...
func loadProviders(autoUpdateDisabled bool, client ProviderClient, path string) ([]catwalk.Provider, error) {
	...
	default:
		...
		if err != nil {
			...
			return nil, fmt.Errorf("Sgptcoder was unable to fetch an updated list of providers from %s. Consider setting SGPT_DISABLE_PROVIDER_AUTO_UPDATE=1 to use the embedded providers bundled at the time of this Sgptcoder release. You can also update providers manually. For more info see sgpt update-providers --help. %w", catwalkUrl, err) //nolint:staticcheck
		}
		return providers, nil
	}
}
