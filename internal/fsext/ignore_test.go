...
func TestSgptIgnore(t *testing.T) {
	...
	// Create a .sgptignore file that ignores .log files
	require.NoError(t, os.WriteFile(".sgptignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .sgptignore file
	sgptignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".sgptignore"), []byte(sgptignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .sgptignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .sgptignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .sgptignore in subdir that ignores normal_nested
	subSgptignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".sgptignore"), []byte(subSgptignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .sgptignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .sgptignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .sgptignore")
