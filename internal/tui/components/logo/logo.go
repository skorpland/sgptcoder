// Package logo renders a Sgptcoder wordmark in a stylized way.
package logo
...
// Opts are the options for rendering the Sgptcoder title art.
type Opts struct {
...
// Render renders the Sgptcoder logo. Set the argument to true to render the narrow
...
func Render(version string, compact bool, o Opts) string {
	...
	sgpt := renderWord(spacing, stretchIndex, letterforms...)
	sgptWidth := lipgloss.Width(sgpt)
	b := new(strings.Builder)
	for r := range strings.SplitSeq(sgpt, "\n") {
	...
	// Charm and version.
	metaRowGap := 1
	maxVersionWidth := sgptWidth - lipgloss.Width(charm) - metaRowGap
	version = ansi.Truncate(version, maxVersionWidth, "…") // truncate version if too long.
	gap := max(0, sgptWidth-lipgloss.Width(charm)-lipgloss.Width(version))
	metaRow := fg(o.CharmColor, charm) + strings.Repeat(" ", gap) + fg(o.VersionColor, version)

	// Join the meta row and big Sgptcoder title.
	sgpt = strings.TrimSpace(metaRow + "\n" + sgpt)

	// Narrow version.
	if compact {
		field := fg(o.FieldColor, strings.Repeat(diag, sgptWidth))
		return strings.Join([]string{field, field, sgpt, field, ""}, "\n")
	}

	fieldHeight := lipgloss.Height(sgpt)
	...
	// Right field.
	rightWidth := max(15, o.Width-sgptWidth-leftWidth-2) // 2 for the gap.
	const stepDownAt = 0
	...
	const hGap = " "
	logo := lipgloss.JoinHorizontal(lipgloss.Top, leftField.String(), hGap, sgpt, hGap, rightField.String())
	if o.Width > 0 {
...
// SmallRender renders a smaller version of the Sgptcoder logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(width int) string {
	t := styles.CurrentTheme()
	title := t.S().Base.Foreground(t.Secondary).Render("Charm™")
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad("Sgptcoder", t.Secondary, t.Primary))
	remainingWidth := width - lipgloss.Width(title) - 1 // 1 for the space after "Sgptcoder"
	if remainingWidth > 0 {
