...
try {
  ...
  console.log("sgptcoder session", session.id)
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/sgptcoder|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/sgptcoder` or `/oc`")
  }
}
...
async function assertOpencodeConnected() {
  ...
  if (!connected) {
    throw new Error("Failed to connect to sgptcoder server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.sgptcoder.ai" : "https://sgptcoder.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.sgptcoder.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("sgptcoder-github-action")
    response = await fetch("https://api.sgptcoder.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  let prompt = (() => {
    const payload = useContext().payload as IssueCommentEvent
    const body = payload.comment.body.trim()
    if (body === "/sgptcoder" || body === "/oc") return "Summarize this thread"
    if (body.includes("/sgptcoder") || body.includes("/oc")) return body
    throw new Error("Comments must mention `/sgptcoder` or `/oc`")
  })()
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to sgptcoder...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "sgptcoder-agent[bot]"`
  await $`git config --global user.email "sgptcoder-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `sgptcoder/${type}${useIssueId()}-${timestamp}`
}
