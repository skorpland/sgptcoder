...
There are two types of agents in sgptcoder; primary agents and subagents.
...
sgptcoder comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
sgptcoder comes with one built-in subagent, **General**. We'll look at this below.

---

## Built-in

sgptcoder comes with two built-in primary agents and one built-in subagent.
...
Configure agents in your `sgptcoder.json` config file:
...
- Global: `~/.config/sgptcoder/agent/`
- Per-project: `.sgptcoder/agent/`

```markdown title="~/.config/sgptcoder/agent/review.md"
---
...
If no temperature is specified, sgptcoder uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global sgptcoder config and the project specific config.
...
Configure permissions globally in sgptcoder.json.
...
```json title="sgptcoder.json" {7-18}
{
  "$schema": "https://sgptcoder.ai/config.json",
  "agent": {
    "build": {
...
:::tip
Run `sgptcoder models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/skorpland/sgptcoder).
:::
...
```markdown title="~/.config/sgptcoder/agent/docs-writer.md"
---
...
```markdown title="~/.config/sgptcoder/agent/security-auditor.md"
---
