---
title: SDK
description: Type-safe JS client for sgptcoder server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The sgptcoder JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control sgptcoder programmatically.
...
You can also programmatically start an sgptcoder server:

```javascript
import { createSgptcoderServer } from "@skorpland/sdk"
...
You can pass a configuration object to customize server behavior. The server still picks up your `sgptcoder.json`, but you can override or add configuration inline:
...
```typescript
import type { Session, Message, Part } from "@skorpland/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*sgptcoder" },
})
