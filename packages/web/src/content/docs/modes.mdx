...
:::caution
Modes are now configured through the `agent` option in the sgptcoder config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in sgptcoder allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the sgptcoder config.
...
sgptcoder comes with two built-in modes.
...
Configure modes in your `sgptcoder.json` config file:
...
- Global: `~/.config/sgptcoder/mode/`
- Project: `.sgptcoder/mode/`

```markdown title="~/.config/sgptcoder/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global sgptcoder config and the project specific config.
...
```json title="sgptcoder.json" {4-14}
{
  "$schema": "https://sgptcoder.ai/config.json",
  "mode": {
    "docs": {
...
Create mode files in `.sgptcoder/mode/` for project-specific modes or `~/.config/sgptcoder/mode/` for global modes:

```markdown title=".sgptcoder/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/sgptcoder/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
