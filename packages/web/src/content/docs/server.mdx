---
title: Server
description: Interact with sgptcoder server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `sgptcoder serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an sgptcoder client can use.
...
```bash
sgptcoder serve [--port <number>] [--hostname <string>]
```
...
When you run `sgptcoder` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the sgptcoder server to interact with sgptcoder programmatically.
:::

This architecture lets sgptcoder support multiple clients and allows you to interact with sgptcoder programmatically.

You can run `sgptcoder serve` to start a standalone server. If you have the
sgptcoder TUI running, `sgptcoder serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the sgptcoder [IDE](/docs/ide) plugins.
...
The sgptcoder server exposes the following APIs.
...
| `POST`   | `/session/:id/message`                   | Send chat message                  | body matches [`ChatInput`](https://github.com/skorpland/sgptcoder/blob/main/packages/sgptcoder/src/session/index.ts#L358), returns <a href={typesUrl}><code>Message</code></a>     |
| `POST`   | `/session/:id/shell`                     | Run a shell command                | body matches [`CommandInput`](https://github.com/skorpland/sgptcoder/blob/main/packages/sgptcoder/src/session/index.ts#L1007), returns <a href={typesUrl}><code>Message</code></a> |
