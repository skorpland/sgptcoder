...
With sgptcoder you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, sgptcoder uses our own `sgptcoder` theme.
...
sgptcoder comes with several built-in themes.
...
- Want sgptcoder to match their terminal's appearance
...
sgptcoder supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/sgptcoder/themes/*.json` or `$XDG_CONFIG_HOME/sgptcoder/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.sgptcoder/themes/*.json`
4. **Current working directory** - Defined in `./.sgptcoder/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/sgptcoder/themes
vim ~/.config/sgptcoder/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .sgptcoder/themes
vim .sgptcoder/themes/my-theme.json
```
...
{
  "$schema": "https://sgptcoder.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
