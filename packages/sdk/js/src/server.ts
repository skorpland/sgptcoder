...
export async function createSgptcoderServer(options?: ServerOptions) {
  ...
  const proc = spawn(`sgptcoder`, [`serve`, `--hostname=${options.hostname}`, `--port=${options.port}`], {
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    proc.stdout?.on("data", (chunk) => {
      ...
      for (const line of lines) {
        if (line.startsWith("sgptcoder server listening")) {
...
export function createSgptcoderTui(options?: TuiOptions) {
  ...
  const proc = spawn(`sgptcoder`, args, {
    signal: options?.signal,
    stdio: "inherit",
    env: {
      ...process.env,
