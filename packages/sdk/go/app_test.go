...
func TestAppLogWithOptionalParams(t *testing.T) {
	...
	_, err := client.App.Log(context.TODO(), sgptcoder.AppLogParams{
		Level:     sgptcoder.F(sgptcoder.AppLogParamsLevelDebug),
		Message:   sgptcoder.F("message"),
		Service:   sgptcoder.F("service"),
		Directory: sgptcoder.F("directory"),
		Extra: sgptcoder.F(map[string]interface{}{
			"foo": "bar",
		}),
	})
	if err != nil {
...
func TestAppProvidersWithOptionalParams(t *testing.T) {
	...
	_, err := client.App.Providers(context.TODO(), sgptcoder.AppProvidersParams{
		Directory: sgptcoder.F("directory"),
	})
	if err != nil {
		var apierr *sgptcoder.Error
