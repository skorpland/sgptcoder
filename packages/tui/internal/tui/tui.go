...
import (
	...
	"github.com/skorpland/sgptcoder/internal/api"
	"github.com/skorpland/sgptcoder/internal/app"
	"github.com/skorpland/sgptcoder/internal/commands"
	"github.com/skorpland/sgptcoder/internal/completions"
	"github.com/skorpland/sgptcoder/internal/components/chat"
	cmdcomp "github.com/skorpland/sgptcoder/internal/components/commands"
	"github.com/skorpland/sgptcoder/internal/components/dialog"
	"github.com/skorpland/sgptcoder/internal/components/modal"
	"github.com/skorpland/sgptcoder/internal/components/status"
	"github.com/skorpland/sgptcoder/internal/components/toast"
	"github.com/skorpland/sgptcoder/internal/layout"
	"github.com/skorpland/sgptcoder/internal/styles"
	"github.com/skorpland/sgptcoder/internal/theme"
	"github.com/skorpland/sgptcoder/internal/util"
)
...
func (a Model) Init() tea.Cmd {
	...
	// https://github.com/skorpland/sgptcoder/issues/127
	if !util.IsWsl() {
...
func (a Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
		...
		if a.app.CurrentPermission.ID != "" {
			if keyString == "enter" || keyString == "esc" || keyString == "a" {
				...
				if len(a.app.Permissions) > 0 {
					...
					a.app.CurrentPermission = sgptcoder.Permission{}
				}
				response := sgptcoder.SessionPermissionRespondParamsResponseOnce
				switch keyString {
				case "enter":
					response = sgptcoder.SessionPermissionRespondParamsResponseOnce
				case "a":
					response = sgptcoder.SessionPermissionRespondParamsResponseAlways
				case "esc":
					response = sgptcoder.SessionPermissionRespondParamsResponseReject
				}
				...
				return a, func() tea.Msg {
					resp, err := a.app.Client.Session.Permissions.Respond(
						...
						sessionID,
						permissionID,
						sgptcoder.SessionPermissionRespondParams{Response: sgptcoder.F(response)},
					)
					if err != nil {
	...
	case app.SendPrompt:
		...
		if a.app.Session.ParentID != "" {
			parentSession, err := a.app.Client.Session.Get(context.Background(), a.app.Session.ParentID, sgptcoder.SessionGetParams{})
			if err != nil {
	...
	case app.SessionClearedMsg:
		a.app.Session = &sgptcoder.Session{}
		a.app.Messages = []app.Message{}
	case dialog.CompletionDialogCloseMsg:
		a.showCompletionDialog = false
	case sgptcoder.EventListResponseEventInstallationUpdated:
		return a, toast.NewSuccessToast(
			"sgptcoder updated to "+msg.Properties.Version+", restart to apply.",
			toast.WithTitle("New version installed"),
		)
		/*
			case sgptcoder.EventListResponseEventIdeInstalled:
				return a, toast.NewSuccessToast(
					"Installed the sgptcoder extension in "+msg.Properties.Ide,
					toast.WithTitle(msg.Properties.Ide+" extension installed"),
				)
		*/
	case sgptcoder.EventListResponseEventSessionDeleted:
		if a.app.Session != nil && msg.Properties.Info.ID == a.app.Session.ID {
			a.app.Session = &sgptcoder.Session{}
			a.app.Messages = []app.Message{}
		}
		return a, toast.NewSuccessToast("Session deleted successfully")
	case sgptcoder.EventListResponseEventSessionUpdated:
		if msg.Properties.Info.ID == a.app.Session.ID {
			a.app.Session = &msg.Properties.Info
		}
	case sgptcoder.EventListResponseEventMessagePartUpdated:
		...
		if msg.Properties.Part.SessionID == a.app.Session.ID {
			messageIndex := slices.IndexFunc(a.app.Messages, func(m app.Message) bool {
				...
				case sgptcoder.AssistantMessage:
					return casted.ID == msg.Properties.Part.MessageID
				}
				return false
			})
			if messageIndex > -1 {
				...
				partIndex := slices.IndexFunc(message.Parts, func(p sgptcoder.PartUnion) bool {
					switch casted := p.(type) {
					case sgptcoder.TextPart:
						return casted.ID == msg.Properties.Part.ID
					case sgptcoder.ReasoningPart:
						return casted.ID == msg.Properties.Part.ID
					case sgptcoder.FilePart:
						return casted.ID == msg.Properties.Part.ID
					case sgptcoder.ToolPart:
						return casted.ID == msg.Properties.Part.ID
					case sgptcoder.StepStartPart:
						return casted.ID == msg.Properties.Part.ID
					case sgptcoder.StepFinishPart:
						return casted.ID == msg.Properties.Part.ID
					}
					return false
				})
	...
	case sgptcoder.EventListResponseEventMessagePartRemoved:
	...
	case sgptcoder.EventListResponseEventMessageRemoved:
	...
	case sgptcoder.EventListResponseEventMessageUpdated:
	...
	case sgptcoder.EventListResponseEventPermissionUpdated:
		...
		a.app.CurrentPermission = a.app.Permissions[0]
		a.editor.Blur()
	case sgptcoder.EventListResponseEventPermissionReplied:
		index := slices.IndexFunc(a.app.Permissions, func(p sgptcoder.Permission) bool {
	...
	case sgptcoder.EventListResponseEventSessionError:
		switch err := msg.Properties.Error.AsUnion().(type) {
		case nil:
		case sgptcoder.ProviderAuthError:
		...
		case sgptcoder.UnknownError:
			slog.Error("Server error", "name", err.Name, "message", err.Data.Message)
			return a, toast.NewErrorToast(err.Data.Message, toast.WithTitle(string(err.Name)))
		}
	case sgptcoder.EventListResponseEventSessionCompacted:
		if msg.Properties.SessionID == a.app.Session.ID {
	...
	case dialog.RestoreToMessageMsg:
		cmd := func() tea.Msg {
			...
			for i := msg.Index + 1; i < len(a.app.Messages); i++ {
				if userMsg, ok := a.app.Messages[i].Info.(sgptcoder.UserMessage); ok {
					nextMessageID = userMsg.ID
					break
				}
			}

			var response *sgptcoder.Session
			var err error

			if nextMessageID == "" {
				// Last message - use unrevert to restore full conversation
				response, err = a.app.Client.Session.Unrevert(context.Background(), a.app.Session.ID, sgptcoder.SessionUnrevertParams{})
			} else {
				...
				response, err = a.app.Client.Session.Revert(context.Background(), a.app.Session.ID,
					sgptcoder.SessionRevertParams{MessageID: sgptcoder.F(nextMessageID)})
			}
...
func (a Model) executeCommand(command commands.Command) (tea.Model, tea.Cmd) {
	...
	case commands.SessionShareCommand:
		...
		response, err := a.app.Client.Session.Share(context.Background(), a.app.Session.ID, sgptcoder.SessionShareParams{})
		if err != nil {
	...
	case commands.SessionUnshareCommand:
		...
		_, err := a.app.Client.Session.Unshare(context.Background(), a.app.Session.ID, sgptcoder.SessionUnshareParams{})
		if err != nil {
	...
	case commands.SessionChildCycleCommand:
		...
		cmds = append(cmds, func() tea.Msg {
			...
			var parentSession *sgptcoder.Session
			if a.app.Session.ParentID != "" {
				parentSessionID = a.app.Session.ParentID
				session, err := a.app.Client.Session.Get(context.Background(), parentSessionID, sgptcoder.SessionGetParams{})
				if err != nil {
					slog.Error("Failed to get parent session", "error", err)
			...
			children, err := a.app.Client.Session.Children(context.Background(), parentSessionID, sgptcoder.SessionChildrenParams{})
			if err != nil {
			...
			sessions := []*sgptcoder.Session{parentSession}
