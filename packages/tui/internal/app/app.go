...
import (
	...
	"github.com/skorpland/sgptcoder/internal/clipboard"
...
type Message struct {
	Info  sgptcoder.MessageUnion
	Parts []sgptcoder.PartUnion
}

type App struct {
	Project           sgptcoder.Project
	Agents            []sgptcoder.Agent
	Providers         []sgptcoder.Provider
	Version           string
	StatePath         string
	Config            *sgptcoder.Config
	Client            *sgptcoder.Client
	State             *State
	AgentIndex        int
	Provider          *sgptcoder.Provider
	Model             *sgptcoder.Model
	Session           *sgptcoder.Session
	Messages          []Message
	Permissions       []sgptcoder.Permission
	CurrentPermission sgptcoder.Permission
...
func (a *App) Agent() *sgptcoder.Agent {
...
type SessionSelectedMsg = *sgptcoder.Session
...
type PermissionRespondedToMsg struct {
	Response sgptcoder.SessionPermissionRespondParamsResponse
}
...
func New(
	...
	configInfo, err := httpClient.Config.Get(ctx, sgptcoder.ConfigGetParams{})
	if err != nil {
		return nil, err
	}
	...
	themeEnv := os.Getenv("SGPTCODER_THEME")
	if themeEnv != "" {
		appState.Theme = themeEnv
	}

	agentIndex := slices.IndexFunc(agents, func(a sgptcoder.Agent) bool {
		return a.Mode != "subagent"
	})
	var agent *sgptcoder.Agent
	modeName := "build"
	if appState.Agent != "" {
	...
	customCommands, err := httpClient.Command.List(ctx, sgptcoder.CommandListParams{})
	if err != nil {
		return nil, err
	}
...
func findModelByFullID(
	providers []sgptcoder.Provider,
	fullModelID string,
) (*sgptcoder.Provider, *sgptcoder.Model) {
...
func findProviderByID(providers []sgptcoder.Provider, providerID string) *sgptcoder.Provider {
	for _, provider := range providers {
		if provider.ID == providerID {
			return &provider
		}
...
func (a *App) InitializeProvider() tea.Cmd {
	providersResponse, err := a.Client.App.Providers(context.Background(), sgptcoder.AppProvidersParams{})
	if err != nil {
	...
	var selectedProvider *sgptcoder.Provider
	var selectedModel *sgptcoder.Model
...
func getDefaultModel(
	response *sgptcoder.AppProvidersResponse,
	provider sgptcoder.Provider,
) *sgptcoder.Model {
...
func (a *App) IsBusy() bool {
	...
	if casted, ok := lastMessage.Info.(sgptcoder.AssistantMessage); ok {
...
func (a *App) HasAnimatingWork() bool {
	for _, msg := range a.Messages {
		...
		for _, p := range msg.Parts {
			if tp, ok := p.(sgptcoder.ToolPart); ok {
				if tp.State.Status == sgptcoder.ToolPartStateStatusPending {
					return true
				}
			}
		}
...
func (a *App) InitializeProject(ctx context.Context) tea.Cmd {
	...
	go func() {
		_, err := a.Client.Session.Init(ctx, a.Session.ID, sgptcoder.SessionInitParams{
			MessageID:  sgptcoder.F(id.Ascending(id.Message)),
			ProviderID: sgptcoder.F(a.Provider.ID),
			ModelID:    sgptcoder.F(a.Model.ID),
		})
		if err != nil {
...
func (a *App) CreateSession(ctx context.Context) (*sgptcoder.Session, error) {
	session, err := a.Client.Session.New(ctx, sgptcoder.SessionNewParams{})
	if err != nil {
		return nil, err
	}
	return session, nil
...
func (a *App) SendPrompt(ctx context.Context, prompt Prompt) (*App, tea.Cmd) {
	...
	cmds = append(cmds, func() tea.Msg {
		_, err := a.Client.Session.Prompt(ctx, a.Session.ID, sgptcoder.SessionPromptParams{
			Model: sgptcoder.F(sgptcoder.SessionPromptParamsModel{
				ProviderID: sgptcoder.F(a.Provider.ID),
				ModelID:    sgptcoder.F(a.Model.ID),
			}),
			Agent:     sgptcoder.F(a.Agent().Name),
			MessageID: sgptcoder.F(messageID),
			Parts:     sgptcoder.F(message.ToSessionChatParams()),
		})
		if err != nil {
...
func (a *App) SendCommand(ctx context.Context, command string, args string) (*App, tea.Cmd) {
	...
	cmds = append(cmds, func() tea.Msg {
		params := sgptcoder.SessionCommandParams{
			Command:   sgptcoder.F(command),
			Arguments: sgptcoder.F(args),
			Agent:     sgptcoder.F(a.Agents[a.AgentIndex].Name),
		}
		if a.Provider != nil && a.Model != nil {
			params.Model = sgptcoder.F(a.Provider.ID + "/" + a.Model.ID)
		}
...
func (a *App) Cancel(ctx context.Context, sessionID string) error {
	...
	_, err := a.Client.Session.Abort(ctx, sessionID, sgptcoder.SessionAbortParams{})
	if err != nil {
...
func (a *App) ListSessions(ctx context.Context) ([]sgptcoder.Session, error) {
	response, err := a.Client.Session.List(ctx, sgptcoder.SessionListParams{})
	if err != nil {
		return nil, err
	}
	if response == nil {
		return []sgptcoder.Session{}, nil
	}
	sessions := *response
	return sessions, nil
}
...
func (a *App) DeleteSession(ctx context.Context, sessionID string) error {
	_, err := a.Client.Session.Delete(ctx, sessionID, sgptcoder.SessionDeleteParams{})
	if err != nil {
...
func (a *App) UpdateSession(ctx context.Context, sessionID string, title string) error {
	_, err := a.Client.Session.Update(ctx, sessionID, sgptcoder.SessionUpdateParams{
...
func (a *App) ListMessages(ctx context.Context, sessionId string) ([]Message, error) {
	response, err := a.Client.Session.Messages(ctx, sessionId, sgptcoder.SessionMessagesParams{})
	if err != nil {
		return nil, err
	}
	if response == nil {
...
func (a *App) ListProviders(ctx context.Context) ([]sgptcoder.Provider, error) {
	response, err := a.Client.App.Providers(ctx, sgptcoder.AppProvidersParams{})
	if err != nil {
		return nil, err
	}
	if response == nil {
		return []sgptcoder.Provider{}, nil
	}
