...
func renderText(
	app *app.App,
	message sgptcoder.MessageUnion,
	text string,
	author string,
	...
	isQueued bool,
	shimmer bool,
	fileParts []sgptcoder.FilePart,
	agentParts []sgptcoder.AgentPart,
	toolCalls ...sgptcoder.ToolPart,
) string {
	t := theme.CurrentTheme()
	...
	if assistantMsg, ok := message.(sgptcoder.AssistantMessage); ok && assistantMsg.Mode != "" {
	...
	if !showToolDetails && toolCalls != nil && len(toolCalls) > 0 {
		for _, toolCall := range toolCalls {
			...
			style := styles.NewStyle()
			if toolCall.State.Status == sgptcoder.ToolPartStateStatusError {
...
func renderToolDetails(
	...
	if toolCall.State.Status == sgptcoder.ToolPartStateStatusPending {
		...
		case "task":
			...
			if summary != nil {
				...
				for _, item := range toolcalls {
					...
					var toolCall sgptcoder.ToolPart
...
func renderToolName(name string) string {
	...
	default:
		normalizedName := name
		if after, ok := strings.CutPrefix(name, "sgptcoder_"); ok {
...
func getTodoTitle(toolCall sgptcoder.ToolPart) string {
	if toolCall.State.Status == sgptcoder.ToolPartStateStatusCompleted {
