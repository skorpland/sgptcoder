...
func TestLoadThemesFromJSON(t *testing.T) {
	...
	expectedThemes := []string{"tokyonight", "sgptcoder", "everforest", "ayu"}
...
func TestLoadThemesFromDirectories(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(userConfig, "sgptcoder", "themes"), 0755)
	os.MkdirAll(filepath.Join(projectRoot, ".sgptcoder", "themes"), 0755)
	os.MkdirAll(filepath.Join(cwd, ".sgptcoder", "themes"), 0755)
	...
	os.WriteFile(filepath.Join(userConfig, "sgptcoder", "themes", "override-test.json"), []byte(testTheme1), 0644)
	os.WriteFile(filepath.Join(projectRoot, ".sgptcoder", "themes", "override-test.json"), []byte(testTheme2), 0644)
	os.WriteFile(filepath.Join(cwd, ".sgptcoder", "themes", "override-test.json"), []byte(testTheme3), 0644)
