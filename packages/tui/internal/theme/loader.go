...
// 2. USER_CONFIG/sgptcoder/themes/*.json
// 3. PROJECT_ROOT/.sgptcoder/themes/*.json
// 4. CWD/.sgptcoder/themes/*.json
func LoadThemesFromDirectories(userConfig, projectRoot, cwd string) error {
	...
	dirs := []string{
		...
		filepath.Join(projectRoot, ".sgptcoder", "themes"),
	}
	if cwd != projectRoot {
		dirs = append(dirs, filepath.Join(cwd, ".sgptcoder", "themes"))
	}
