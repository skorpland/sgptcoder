...
function findBinary() {
  ...
  const packageName = `sgptcoder-${platform}-${arch}`
  const binary = platform === "windows" ? "sgptcoder.exe" : "sgptcoder"
...
async function regenerateWindowsCmdWrappers() {
  ...
  try {
    ...
    const cmd = `npm rebuild sgptcoder-ai --ignore-scripts${isGlobal ? " -g" : ""}`
    const opts = {
      stdio: "inherit",
      shell: true,
    ...
    console.error("npm rebuild failed. You may need to manually run: npm rebuild sgptcoder-ai --ignore-scripts")
  }
}
...
async function main() {
  try {
    ...
    const binScript = path.join(__dirname, "bin", "sgptcoder")
    ...
    console.log(`sgptcoder binary symlinked: ${binScript} -> ${binaryPath}`)
  } catch (error) {
    console.error("Failed to create sgptcoder binary symlink:", error.message)
    process.exit(1)
  }
}
