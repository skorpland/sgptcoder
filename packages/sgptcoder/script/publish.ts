...
for (const [os, arch] of targets) {
  ...
  // await $`bun build --define OPENCODE_TUI_PATH="'../../../dist/${name}/bin/tui'" --define OPENCODE_VERSION="'${version}'" --compile --target=bun-${os}-${arch} --outfile=dist/${name}/bin/sgptcoder ./src/index.ts`
...
if (!snapshot) {
  ...
  const sourcePkgbuild = [
    ...
    "  cd ../sgptcoder",
    `  bun build --define OPENCODE_TUI_PATH="'$(realpath ../tui/tui)'" --define OPENCODE_VERSION="'\${pkgver}'" --compile --target=bun-linux-x64 --outfile=sgptcoder ./src/index.ts`,
    "}",
    "",
    "package() {",
