...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    for (const file of ["sgptcoder.jsonc", "sgptcoder.json"]) {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        const wellknown = await fetch(`${key}/.well-known/sgptcoder`).then((x) => x.json())
    ...
    const markdownAgents = [
      ...
      ...(await Filesystem.globUp(".sgptcoder/agent/**/*.md", Instance.directory, Instance.worktree)),
    ]
    for (const item of markdownAgents) {
      ...
      const agentFolderPath = item.includes("/.sgptcoder/agent/")
        ? item.split("/.sgptcoder/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
    ...
    const markdownModes = [
      ...
      ...(await Filesystem.globUp(".sgptcoder/mode/*.md", Instance.directory, Instance.worktree)),
    ]
    ...
    const markdownCommands = [
      ...
      ...(await Filesystem.globUp(".sgptcoder/command/**/*.md", Instance.directory, Instance.worktree)),
    ]
    for (const item of markdownCommands) {
      ...
      const name = (() => {
        const patterns = ["/.sgptcoder/command/", "/command/"]
    ...
    result.plugin.push(
      ...[
        ...
        ...(await Filesystem.globUp(".sgptcoder/plugin/*.{ts,js}", Instance.directory, Instance.worktree)),
      ].map((x) => "file://" + x),
    )
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://sgptcoder.ai/docs/commands"),
      watcher: z
        .object({
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://sgptcoder.ai/docs/agent"),
      provider: z
        .record(
          z.string(),
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "sgptcoder.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "sgptcoder.jsonc"))),
    )
    ...
    await import(path.join(Global.Path.config, "config"), {
      ...
      .then(async (mod) => {
        ...
        result["$schema"] = "https://sgptcoder.ai/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://sgptcoder.ai/config.json"
