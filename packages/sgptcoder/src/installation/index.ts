...
export namespace Installation {
  ...
  export async function method() {
    if (process.execPath.includes(path.join(".sgptcoder", "bin"))) return "curl"
    ...
    const checks = [
      ...
      {
        name: "brew" as const,
        command: () => $`brew list --formula sgptcoder-ai`.throws(false).text(),
      },
    ]
    ...
    for (const check of checks) {
      ...
      if (output.includes("sgptcoder-ai")) {
        return check.name
      }
    }
  ...
  export async function upgrade(method: Method, target: string) {
    const cmd = (() => {
      switch (method) {
        case "curl":
          return $`curl -fsSL https://sgptcoder.ai/install | bash`.env({
            ...process.env,
            VERSION: target,
          })
        case "npm":
          return $`npm install -g sgptcoder-ai@${target}`
        case "pnpm":
          return $`pnpm install -g sgptcoder-ai@${target}`
        case "bun":
  ...
  export async function latest() {
    return fetch("https://api.github.com/repos/skorpland/sgptcoder/releases/latest")
      .then((res) => res.json())
      .then((data) => {
